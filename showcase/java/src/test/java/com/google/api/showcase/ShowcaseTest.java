...
public class ShowcaseTest {
  ...
  public void quotaProjectIdTest() throws Exception {
    ...
    final GoogleCredentials credentialsWithQuotaProject = loadCredentials(JSON_KEY_QUOTA_PROJECT_ID);
    final CredentialsProvider credentialsProviderWithQuota = new CredentialsProvider() {
      @Override
      public Credentials getCredentials() throws IOException {
        return credentialsWithQuotaProject;
      }
    };
    final HeaderProvider headerProviderWithQuota = new HeaderProvider() {
      @Override
      public Map<String, String> getHeaders() {
        return Collections.singletonMap(QUOTA_PROJECT_ID_KEY, QUOTA_PROJECT_ID_FROM_HEADER_VALUE);
      }
    };

    EchoSettings settingsSetQuota = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(NoCredentialsProvider.create()).setQuotaProjectId(QUOTA_PROJECT_ID).build();
    EchoSettings settingsHeader = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(NoCredentialsProvider.create()).setHeaderProvider(headerProviderWithQuota).build();
    EchoSettings settingsHeaderAndQuota = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(NoCredentialsProvider.create()).setHeaderProvider(headerProviderWithQuota)
        .setQuotaProjectId(QUOTA_PROJECT_ID).build();
    EchoSettings settingsCredentials = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(credentialsProviderWithQuota).build();
    EchoSettings settingsCredentialsAndQuota = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(credentialsProviderWithQuota).setQuotaProjectId(QUOTA_PROJECT_ID).build();
    EchoSettings settingsQuotaAll = EchoSettings.newBuilder().setTransportChannelProvider(channelProvider)
        .setCredentialsProvider(credentialsProviderWithQuota).setQuotaProjectId(QUOTA_PROJECT_ID)
        .setHeaderProvider(headerProviderWithQuota).build();
