 ...
 */
public class JavaViewModelSnippetSetRunner implements LanguageDirectSnippetSetRunner<JavaClass> {

  /**
   * The path to the root of snippet resources.
   */
  private static final String SNIPPET_RESOURCE_ROOT =
      JavaViewModelSnippetSetRunner.class.getPackage().getName().replace('.', '/');

  @Override
  public GeneratedResult generate(JavaClass javaClass, String snippetFileName) {
    ...
    String outputFilename = javaClass.getFileName();
    Doc result = snippets.generateClass(javaClass);
    return GeneratedResult.create(result, outputFilename);
  }

  private interface JavaViewModelSnippetSet {
    Doc generateClass(JavaClass javaClass);
  }
}
