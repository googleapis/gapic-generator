...
/**
 * The GapicProvider which runs Gapic code generation for Node.JS.
 */
public class NodeJSGapicProvider<InputElementT extends ProtoElement>
    implements GapicProvider<InputElementT> {

  private final NodeJSGapicContext context;
  private final NodeJSSnippetSetRunner snippetSetRunner;
  private InputElementView<InputElementT> view;

  public NodeJSGapicProvider(
      Model model, ApiConfig apiConfig, InputElementView<InputElementT> view) {
    this.context = new NodeJSGapicContext(model, apiConfig);
