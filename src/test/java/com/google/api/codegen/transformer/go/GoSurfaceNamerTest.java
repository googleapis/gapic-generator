...
public class GoSurfaceNamerTest {
  ...
  public void testClientNamePrefix() {
    ...
    assertThat(namer.getPackageName()).isEqualTo("cloud.google.com/go/gopher/apiv1");
    assertThat(namer.getLocalPackageName()).isEqualTo("gopher");

    // Both the service name and the local package name are "gopher",
    // the client name prefix should be empty.
    assertThat(namer.getReducedServiceName("Gopher")).isEqualTo(Name.from("gopher"));
    assertThat(namer.clientNamePrefix("Gopher")).isEqualTo(Name.from());
    ...
    assertThat(namer.getReducedServiceName("GoPher")).isEqualTo(Name.from("go", "pher"));
    assertThat(namer.clientNamePrefix("GoPher")).isEqualTo(Name.from());

    // The service name is different from the local package name,
    // use the service name as the prefix.
    assertThat(namer.getReducedServiceName("Guru")).isEqualTo(Name.from("guru"));
    assertThat(namer.clientNamePrefix("Guru")).isEqualTo(Name.from("guru"));
  }

  @Test
  public void testOperationName() {
    GoSurfaceNamer namer = new GoSurfaceNamer("cloud.google.com/go/gopher/apiv1");
    assertThat(namer.getAndSaveOperationResponseTypeName("CreateStuff"))
        .isEqualTo("CreateStuffOperation");
    assertThat(namer.getAndSaveOperationResponseTypeName("CreateStuffOperation"))
  ...
  public void testPutDocMethodName() {
    GoSurfaceNamer namer = new GoSurfaceNamer("cloud.google.com/go/gopher/apiv1");
    assertThat(namer.putDocMethodName("Create", "creates things", VisibilityConfig.PUBLIC))
