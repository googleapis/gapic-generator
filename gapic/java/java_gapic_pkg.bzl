...
def _java_gapic_build_resources_pkg_impl(ctx):
    ...
    # must ensure uniquness within a build. The template output directory manipulations are
    # to modify defaul 555 file permissions on any generated by bazel file (exectuable read-only,
    # which is not at all what we need for build files). There is no bazel built-in way to change
    # the generated files pemissions also the actual files accessible by the script are symlinks and
    # `chmod`, when applied to a directory, does not change the attributes of symlink targets inside
    # the directory.
    script = """
...
def _construct_package_dir_paths(attr_package_dir, out_pkg, label_name):
    ...
    # (labes are guaranteed to be unique by bazel itself)
